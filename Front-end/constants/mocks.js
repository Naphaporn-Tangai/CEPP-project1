const VideoClip = [
  {
    id: "0",
    idvideo: "VRh5OyhLvLQ",
  },
  {
    id: "1",
    idvideo: "VRh5OyhLvLQ",
  },
  {
    id: "2",
    idvideo: "Wx0WC-6s3h4",
  },
  {
    id: "3",
    idvideo: "Wx0WC-6s3h4",
  },
  {
    id: "4",
    idvideo: "Wx0WC-6s3h4",
  },
];

const UserPick = [
  {
    id: "bd7acbea-c1b1-46c2-aed5-3ad53abb28ba",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
    tool: "เครื่องวัดชีพจร",
    date: "10/01/66",
    toolShow: "เครื่องวัดชีพจร",
    photo:"http://thai.surgicalledlights.com/photo/pl16571502-12_1_inch_6_paramters_portable_vital_signs_monitoring_machine_for_icu_patient.jpg",
    datePick: "10/10/65",
    dateRe: "15/10/65",
    numTool: "1",
    badge: "คืนแล้ว",
    colorBadge: "success"
  },
  {
    id: "3ac68afc-c605-48d3-a4f8-fbd91aa97f63",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
    tool: "เครื่องวัดชีพจร",
    date: "10/01/66",
    toolShow: "เครื่องวัดชีพจร",
    photo:"http://thai.surgicalledlights.com/photo/pl16571502-12_1_inch_6_paramters_portable_vital_signs_monitoring_machine_for_icu_patient.jpg",
    datePick: "10/10/65",
    dateRe: "15/10/65",
    numTool: "2",
    badge: "คืนแล้ว",
    colorBadge: "success"
  },
  {
    id: "58694a0f-3da1-471f-bd96-145571e29d72",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
    tool: "เครื่องวัดชีพจร",
    date: "10/01/66",
    toolShow: "เครื่องวัดชีพจร",
    photo:"http://thai.surgicalledlights.com/photo/pl16571502-12_1_inch_6_paramters_portable_vital_signs_monitoring_machine_for_icu_patient.jpg",
    datePick: "10/10/65",
    dateRe: "15/10/65",
    numTool: "3",
    badge: "คืนแล้ว",
    colorBadge: "success"
  },
  {
    id: "68694a0f-3da1-431f-bd56-142371e29d74",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
    tool: "เครื่องวัดชีพจร",
    date: "10/01/66",
    toolShow: "เครื่องวัดชีพจร",
    photo:"http://thai.surgicalledlights.com/photo/pl16571502-12_1_inch_6_paramters_portable_vital_signs_monitoring_machine_for_icu_patient.jpg",
    datePick: "10/10/65",
    dateRe: "15/10/65",
    numTool: "4",
    badge: "คืนแล้ว",
    colorBadge: "success"
  },
  {
    id: "28694a0f-3da1-471f-bd96-142456e29d77",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
    tool: "เครื่องวัดชีพจร",
    date: "10/01/66",
    toolShow: "เครื่องวัดชีพจร",
    photo:"http://thai.surgicalledlights.com/photo/pl16571502-12_1_inch_6_paramters_portable_vital_signs_monitoring_machine_for_icu_patient.jpg",
    datePick: "10/10/65",
    dateRe: "15/10/65",
    numTool: "5"  ,
     badge: "คืนแล้ว",
    colorBadge: "success"
  },
];

const UserCG = [
  {
    id: "bd7acbea-c1b1-46c2-aed5-3ad53abb28ba",
    fullName: "นางสาวสร้อย  ฟ้าใส",
    title: "Care giver",
    avatarUrl:
      "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
  },
  {
    id: "3ac68afc-c605-48d3-a4f8-fbd91aa97f63",
    fullName: "นายทรัพย์  ทองมี",
    title: "Care giver",
    avatarUrl:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyEaZqT3fHeNrPGcnjLLX1v_W4mvBlgpwxnA&usqp=CAU",
  },
  {
    id: "58694a0f-3da1-471f-bd96-145571e29d72",
    fullName: "นายบัวขาว  อาจอง",
    title: "Care giver",
    avatarUrl: "https://miro.medium.com/max/140/0*0fClPmIScV5pTLoE.jpg",
  },
  {
    id: "68694a0f-3da1-431f-bd56-142371e29d74",
    fullName: "นายทรัพย์  ทองมาก",
    title: "Care giver",
    avatarUrl:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSr01zI37DYuR8bMV5exWQBSw28C1v_71CAh8d7GP1mplcmTgQA6Q66Oo--QedAN1B4E1k&usqp=CAU",
  },
  {
    id: "28694a0f-3da1-471f-bd96-142456e29d77",
    fullName: "นายบัวชมพู  อาจอง",
    title: "Care giver",
    avatarUrl:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRBwgu1A5zgPSvfE83nurkuzNEoXs9DMNr8Ww&usqp=CAU",
  },
];
const WorkofCG = [
  {
    id: "0",
    cgName: "นางสาวสร้อย  ฟ้าใส",
    goWork: "08:00",
    backWork: "18:00",
    SignatureElderly: "6:00",
  },
  {
    id: "1",
    cgName: "นายทรัพย์  ทองมี",
    goWork: "08:00",
    backWork: "08:00",
    SignatureElderly:
      "https://signaturely.com/wp-content/uploads/2020/04/unreadable-letters-signaturely.svg",
  },
  {
    id: "2",
    cgName: "นายบัวขาว  อาจอง",
    goWork: "08:00",
    backWork: "17:00",
    SignatureElderly:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ab/XXXTentacion_signature.svg/1206px-XXXTentacion_signature.svg.png",
  },
  {
    id: "3",
    cgName: "นายบัวชมพู  อาจอ",
    goWork: "08:00",
    backWork: "18:00",
    SignatureElderly:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/Wonho-Signature.svg/2560px-Wonho-Signature.svg.png",
  },
  {
    id: "4",
    cgName: "เช็ดตัว",
    goWork: "08:00",
    backWork: "#35609C",
    timeEvent: "13:00",
  },
];
const Event = [
  {
    id: "0",
    title: "เข้าปฎิบัติงาน",
    num: "ครั้งเดียว",
    Bg: "#35609C",
    timeEvent: "6:00",
  },
  {
    id: "1",
    title: "วัดสัญญาณชีพ",
    num: "08:00",
    Bg: "#8AA7CF",
    timeEvent: "8:00",
  },
  {
    id: "2",
    title: "ทำการกายภาพบำบัด",
    num: "ทุกวันและทุกสัปดาห์",
    Bg: "#7296C7",
    timeEvent: "10:00",
  },
  {
    id: "3",
    title: "ฝึกเดิน",
    num: "ทุกวันและทุกสัปดาห์",
    Bg: "#346CB6",
    timeEvent: "11:00",
  },
  {
    id: "4",
    title: "เช็ดตัว",
    num: "ทุกวันและทุกสัปดาห์",
    Bg: "#35609C",
    timeEvent: "13:00",
  },
];
export { VideoClip, UserCG, UserPick, Event };
